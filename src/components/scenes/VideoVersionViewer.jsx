...
export const VideoVersionViewer = ({ 
  ...
  const [hoveredStoredImage, setHoveredStoredImage] = useState(null);
  const [uploadingStoredImage, setUploadingStoredImage] = useState(null);
  const fileInputRef = useRef(null);

  // Image loading handlers with 400ms delay
  ...
  const handleDownloadWrapper = async (mediaUrl, sceneId, version, isMusic = false) => {
    const extension = isMusic ? 'mp3' : 'mp4';
    const mediaType = isMusic ? 'music' : 'video';
    const filename = getDownloadFilename(project.name || 'untitled', sceneId, version, extension);
    
    await handleDownloadMedia(mediaUrl, filename, mediaType);
  };
  
  // Get versions from the scene data (video or music depending on scene type)
  const videoVersions = sceneData?.videoVersions || [];
  const musicVersions = sceneData?.musicVersions || [];
  
  // Check if we have stored image data for regeneration
  const storedImageData = sceneData?.storedImage;
  
  // Trigger loading state when switching between pages (when storedImageData.url changes)
  useEffect(() => {
    if (storedImageData?.url) {
      ...
      // Set timer to hide loading after 400ms delay
      const timer = setTimeout(() => {
        setImageLoading(false);
      }, 2500); // Show loading for 2.5 seconds to cover the typical load time
  ...
  // Use appropriate versions based on scene type
  let allVersions = sceneId === 'music' ? musicVersions : videoVersions;
  ...
  } else if (storedImageData?.url && videosWithUrls.length === 0 && sceneId !== 'end' && sceneId !== 'music') {
    // Show stored image for regular story scenes when no videos exist
    totalCards.push({
      id: 'stored-image',
      version: 1,
      isStoredImageCard: true,
  ...
  const handleFileInput = (e) => {
    ...
    if (file && onImageUpload) {
      // Pass callback to clear uploading state when upload completes
      onImageUpload(file, () => {
        setUploadingStoredImage(null);
        if (onUploadComplete) onUploadComplete();
      });
    } else {
      // Clear uploading state if user cancelled file selection
      setUploadingStoredImage(null);
    }
  };
  ...
  const renderCard = (card, index) => {
    ...
    if (card.isStoredImageCard) {
      ...
      return (
        <div key={card.id} className="space-y-4">
          <div
            className={`
              relative bg-gray-900 rounded-xl overflow-hidden transition-all duration-300 w-full cursor-pointer group
              border-2 border-solid border-gray-600 hover:border-blue-500
              ${aspectRatioClass}
            `}
            style={{ minHeight: '120px' }}
            onMouseEnter={() => setHoveredStoredImage(card.id)}
            onMouseLeave={() => setHoveredStoredImage(null)}
            onClick={() => {
              if (onImageUpload) {
                // Set uploading state when user clicks to upload
                setUploadingStoredImage(card.id);
                fileInputRef.current?.click();
              }
            }}
          >
            ...
            {uploadingStoredImage === card.id && (
              <div className="absolute inset-0 bg-black/70 backdrop-blur-sm flex flex-col items-center justify-center transition-opacity duration-200 z-30">
                <Loader2 className="animate-spin text-blue-400 mb-2" size={32} />
                <p className="text-white font-medium text-center px-4">
                  Uploading Custom Image...
                </p>
                <p className="text-gray-300 text-sm text-center px-4 mt-1">
                  Please wait
                </p>
              </div>
            )}
            
            {/* Hover overlay for custom image upload */}
            {hoveredStoredImage === card.id && onImageUpload && !uploadingStoredImage && (
              <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center transition-opacity duration-200 z-20">
          ...
          {onGenerate && (
            <button
              ...
              className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white px-6 py-3 rounded-lg font-medium transition-colors duration-200"
            >
              {isUploading ? uploadLoadingMessage : (sceneId === 'cover' ? 'Generate Cover Video' : 'Generate Video')}
            </button>
          )}
        </div>
      );
    ...
    if (card.isLoading) {
      const displayMessage = uploadLoadingMessage || (sceneId === 'music' ? 'Generating music...' : 'Generating video...');
      // Extract progress percentage from message
      let progressPercentage = 0;
      const progressMatch = displayMessage.match(/(\d+)%/);
      if (progressMatch) {
      ...
      return (
        ...
        >
          <div className="relative w-full h-full">
            ...
            {/* Loading overlay for generation state */}
            {imageLoading && (
              <div className="absolute inset-0 bg-white/20 backdrop-blur-sm flex items-center justify-center z-10">
                <Loader2 className="animate-spin text-white" size={32} />
              </div>
            ...
            <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex gap-1">
              ...
              <button
                onClick={(e) => {
                  ...
                  handleDownloadWrapper(mediaUrl, sceneId, card.version, isMusic);
                }}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={(e) => {
                        ...
                        handleDownloadWrapper(mediaUrl, sceneId, selectedVideo.version, isModalMusic);
                      }}
